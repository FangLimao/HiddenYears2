...
import * as dave from '@quest/config.js';
...
const formAbout = (player) => {
  const form = new ActionFormData().title(getTitle).body(`§e运行: §r${systemEnable}\n§e模组作者: §r@${getAuthor}\n§e本任务书基于 §r@abcdave 的 myQuestAPI开发而成`).button(getButtonBack);
  form.show(player).then(response => {
    if (response.selection === 0) {
      formMain(player);
    }
